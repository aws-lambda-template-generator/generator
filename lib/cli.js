...
const main = async() => {
  ...
  // Getting choices
  // (1) get framework choice
  let frameworkId = await promptManager(promptBuilder.getFrameworkPrompt());
  frameworkId = frameworkId === '' ? '1' : frameworkId;
  // (2) get language choice
  let languageId = await promptManager(promptBuilder.getLanguagePrompt());
  languageId = languageId === '' ? '1' : languageId;
  // (3) get example choice
  let exampleId = await promptManager(promptBuilder.getExamplePrompt(frameworkId, languageId));
